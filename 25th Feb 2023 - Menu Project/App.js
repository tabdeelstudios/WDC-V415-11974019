...
const App = () => {
  ...
  return (
    <div>
      <div>
        <button onClick={() => showDishes("all")}>All</button>
        <button onClick={() => showDishes("breakfast")}>Breakfast</button>
        <button onClick={() => showDishes("dinner")}>Dinner</button>
        <button onClick={() => showDishes("shakes")}>Shakes</button>
        <button onClick={() => showDishes("lunch")}>Lunch</button>
      </div>
      <div id="dishes">
        {menu.map(
          (item) =>
            (selectedCategory === "all" ||
              ...
              <div key={item.id} className="dish">
                ...
                <div>
                  <div className="inner">
                    <h3 className="dishName">{item.title}</h3>
                    <h3 className="dishPrice">Rs. {item.price}</h3>
                  </div>
                  <hr />
                  <p className="dishContent">{item.desc}</p>
                </div>
              </div>
            )
        )}
